  ...
  for(var i = 0; i < this.bricks.brickRows; i++) {
    for(var j = 0; j < this.bricks.brickColumns; j++) {
      ...
      if(eachBrick.status === 1) {
        if(this.ball.x >= eachBrick.x && this.ball.x - this.ball.radius <= eachBrick.x + this.bricks.width && this.ball.x + this.ball.radius >= eachBrick.x) {
          if(this.ball.y >= eachBrick.y && this.ball.y - this.ball.radius <= eachBrick.y + this.bricks.height && this.ball.y + this.ball.radius >= eachBrick.y) {
            eachBrick.status = 0;
