  ...
  for(var i = 0; i < this.bricks.brickRows; i++) {
    for(var j = 0; j < this.bricks.brickColumns; j++) {
      ...
      if(eachBrick.status === 1) {
        // this.testForBrickDetectionLeft(eachBrick);
        // this.testForBrickDetectionRight(eachBrick);
...
Collisions.prototype.testForBrickDetectionLeft = function(eachBrick){
  if(this.ball.y + this.ball.radius >= eachBrick.y && this.ball.y <= eachBrick.y + this.bricks.height){
    if((this.ball.x < eachBrick.x && (this.ball.x + this.ball.radius) >= eachBrick.x)){
      eachBrick.status = 0;
...
Collisions.prototype.testForBrickDetectionRight = function(eachBrick){
  if(this.ball.y - this.ball.radius >= eachBrick.y && this.ball.y <= eachBrick.y + this.bricks.height){
    if(this.ball.x > eachBrick.x + eachBrick.width && this.ball.x - this.ball.radius <= (eachBrick.x + eachBrick.width)) {
      eachBrick.status = 0;
      this.changeBallDirectionX();
      this.increaseScore();
      this.decreaseBrickCount();
